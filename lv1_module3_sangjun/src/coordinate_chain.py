...
class CoordinateChain:
    ...
    def _path_to_root(self, frame: str) -> list[str]:
        ...
        while curr != self.root:
            if curr not in self._parent:
                raise KeyError(f"Frame {curr} is not connected to root {self.root}")
    ...
    def T_from_root(self, frame: str) -> np.ndarray:
        ...
        T_res = np.eye(4, dtype=float)
        for parent, child in reversed(pairs):
            T_res = T_res @ self._T[(parent, child)]
        return T_res
