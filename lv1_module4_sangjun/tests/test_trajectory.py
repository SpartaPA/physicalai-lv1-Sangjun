...
def test_linear_interp_hits_waypoints(waypoints):
    ...
    t_wp, q_wp = waypoints
    actual = linear_interp(t_wp, q_wp, t_wp)
    assert actual.shape == q_wp.shape
    assert np.allclose(actual, q_wp)
...
def test_cubic_spline_hits_waypoints(waypoints):
    ...
    t_wp, q_wp = waypoints
    actual = cubic_spline_interp(t_wp, q_wp, t_wp)
    assert actual.shape == q_wp.shape
    assert np.allclose(actual, q_wp)
...
def test_quintic_boundary_conditions():
    ...
    assert np.isclose(q[0], 0.0) and np.isclose(q[-1], 1.0)
    assert np.allclose(qd[[0, -1]], 0.0)
    assert np.allclose(qdd[[0, -1]], 0.0)
