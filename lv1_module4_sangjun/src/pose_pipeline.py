...
class PosePipeline:
    ...
    def __init__(self, T_base_link, T_link_camera, joint_axis: str = "z"):
        ...
        if T_base_link.shape != (4, 4) or T_link_camera.shape != (4, 4):
            raise ValueError("T_base_link 와 T_link_camera 는 (4, 4)이어야 합니다.")
        if joint_axis not in _ROT:
            raise ValueError("joint_axis 는 'x', 'y', 'z' 중 하나여야 합니다.")
    ...
    def T_base_link(self) -> np.ndarray:
        ...
        """
        # TODO: 문제 2-2
        R_joint = _ROT[self.joint_axis](self.joint_angle)
        return self._T_base_link0 @ make_T(R_joint, np.zeros(3))

    @property
    def T_link_camera(self) -> np.ndarray:
        """T(link <- camera) — 카메라는 link 에 고정돼 있으므로 관절과 무관하다."""
        # TODO: 문제 2-1
        return self._T_link_camera.copy()
    ...
    def object_pose_in_base(self, T_camera_object) -> np.ndarray:
        ...
        if T_camera_object.shape != (4, 4):
            raise ValueError("T_camera_object 는 (4, 4)이어야 합니다.")
